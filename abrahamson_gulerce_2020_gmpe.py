...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 15 15:21:30 2024

@author: nbijelic

Changes in the code compared to openQuake

const.TRT.SUBDUCTION_INTERFACE --> 'subduction_interface'
...
ctx.vs30
ctx.ztor
!! Make sure I am using consistent units -- my scripts are VS30 in m, R in km,
 ztor in ??
!! The GMPE assumes ??
...
# Abrahamson Gulerce 2020 GMPE for subduction zone events
...
class AbrahamsonGulerce2020(object):
    """
    Implements the 2020 GMPE for subduction interface and in-slab events based 
    on:
    ...
    url: https://docs.openquake.org/oq-engine/3.18/reference/_modules/openquake/hazardlib/gsim/abrahamson_gulerce_2020.html#AbrahamsonGulerce2020SInter
    
    
    Parameters
    ----------
    T : np array float, periods for which to compute the median and sigma

    Rup : rupture object with following atributes:
        TODO ::: for the GMPE
        TODO ::: trt attribute goes here : 'subduction_interface', 'subduction_inslab'
        
    Site : site object with following attributes:
        TODO ::: for the GMPE
        TODO ::: region should be specified here : "GLO",
                      "USA-AK", "CAS", "CAM", "JPN", "NZL", "SAM", "TWN"
        TODO ::: apply_usa_adjustment (bool) should be defined here:
            Apply the modeller designated Alaska or Cascadia adjustments 
            (available only for the regions "USA-AK" or "CAS")

    Returns
    -------

    """
    def __init__(self, T, Rup, Site, ergodic=True, apply_usa_adjustment=True,
                 sigma_mu_epsilon=0.0, is_scale_to_rotd100 = True, **kwargs):
    ...
    def get_median_and_sigma(self, T, Rup, Site): #compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        ctx is a RuptureContext object or a numpy recarray of size N
            https://docs.openquake.org/oq-engine/3.18/reference/_modules/openquake/hazardlib/gsim/base.html
        ...
        for m, imt in enumerate(imts):
            C = self.COEFFS(imt)#self.COEFFS[imt]
            ...
            tau_m, phi_m = get_tau_phi(C, C_PGA, self.region, imt, #imt.period,
                                       ctx.rrup, ctx.vs30, pga1000,
                                       self.ergodic)
            tau[m] = tau_m
            phi[m] = phi_m

        # sig += np.sqrt(tau ** 2.0 + phi ** 2.0) # was this an error
        sig = np.sqrt(tau ** 2.0 + phi ** 2.0)

        
        # median = np.exp(mean[0])
        # sigma = sig[0]
        ...
        # SOMETHING IS WRONG HERE ... debug this part !!!
